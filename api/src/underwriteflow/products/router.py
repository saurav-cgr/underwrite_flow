...
def parse_configuration(payload: YamlPayload):
    try:
        ...
        raise HTTPException(status_code=422, detail="Invalid product configuration") from None
...
async def validate_configuration(
    payload: YamlPayload,
    _: dict[str, str] = Depends(require_permission(Permission.PRODUCT_CONFIG_WRITE)),
) -> dict[str, str]:
    configuration = parse_configuration(payload)
    return {"product_code": configuration.product_code, "version": configuration.version}
...
async def import_configuration(
    payload: YamlPayload,
    admin: dict[str, str] = Depends(require_permission(Permission.PRODUCT_CONFIG_WRITE)),
    session: AsyncSession = Depends(get_session),
) -> dict[str, str]:
    configuration = parse_configuration(payload)
    try:
        version = await ProductService().import_configuration(
            session, configuration, UUID(admin["sub"])
        )
...
async def list_history(
    product_code: str,
    _: dict[str, str] = Depends(require_permission(Permission.PRODUCT_CONFIG_READ)),
    session: AsyncSession = Depends(get_session),
) -> list[dict[str, str | None]]:
    versions = await ProductService().repository.list_versions(session, product_code)
    return [
        {
            "version": version.version,
            "status": version.status,
...
async def activate_configuration(
    ...
    try:
        version = await ProductService().activate(
            session, product_code, payload.version, UUID(admin["sub"])
        )
    except ProductConfigurationError as error:
        raise HTTPException(status_code=404, detail=str(error)) from None
    return {"product_code": product_code, "version": version.version, "status": version.status}
