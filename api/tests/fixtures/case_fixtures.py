"""Case-scoped database and upload-volume helpers for the test suites.

Split out of ``records.py`` to keep that file under the project's 400-line
limit. Identity and role helpers stay in ``records.py``; case, product-status,
and evidence helpers live here.
"""
...
# A complete new-business application, so a seeded case is submit-ready.
SEED_APPLICATION_PAYLOAD = (
    '{"application": {"vehicle_age": 2, "vehicle_use": "personal", '
    '"prior_claims": 0}}'
)
...
def set_motor_status(status: str, version: str = "v1") -> None:
    with psycopg.connect(DATABASE_URL) as connection:
        with connection.cursor() as cursor:
            if status == "active":
                cursor.execute(
                    "UPDATE product_versions SET status = 'draft' WHERE "
                    "product_id = (SELECT id FROM products WHERE code = %s)",
                    ("motor-private-car",),
                )
            cursor.execute(
