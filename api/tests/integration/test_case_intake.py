...
# Verify idempotent intake, safe upload metadata, and unsupported-type rejection.
def test_case_intake_is_idempotent_and_stores_safe_document_metadata() -> None:
    ...
    try:
        application = {
            ...
            "payload": {"vehicle_age": 2, "vehicle_use": "personal", "prior_claims": 0},
        ...
        with TestClient(create_app()) as client:
            ...
            created = client.post("/api/v1/cases", json=application, headers=headers)
            repeated = client.post("/api/v1/cases", json=application, headers=headers)
            ...
            assert document.json()["content_hash"] == hashlib.sha256(content).hexdigest()
