...
# Verify one selected product subgraph runs checks in stable order.
@pytest.mark.asyncio
async def test_selected_product_subgraph_only_processes_selected_product(
) -> None:
    configurations = {
    ...
    result = await graph.ainvoke(
        {
            ...
            "payload": {"requested_cover": 2},
            "rule_results": [],
        }
    )

    assert [
        item["rule_code"] for item in result["validations"]
    ] == ["life_rule"]
...
# Given one invalid rule, preserve its successful sibling result.
@pytest.mark.asyncio
